...
type WorkloadSignature struct {
	...
	// ShardMetadata는 디렉터리/tar 스캔 상세(경로 샘플, 엔트리 수 등)를 담는다.
	ShardMetadata *ShardInputScanResult `json:"shard_metadata,omitempty"`
...
// ShardInputScanResult는 디렉터리 또는 tar/tar.gz 아카이브를 읽어 계산한 shard 입력 메타이다.
type ShardInputScanResult struct {
	...
	ShardEntries   []string `json:"shard_entries,omitempty"`
	ShardSource    string   `json:"shard_source"`
	ShardScanError string   `json:"shard_scan_error,omitempty"`
	TarHeaderCount int      `json:"tar_header_count,omitempty"`
}
